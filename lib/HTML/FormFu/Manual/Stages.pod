...
Here we use the C<default_values> method of the model associated by the form, because potentially the result object can be a very complex datastructure, and the model's L<HTML::FormFu::Model::DBIC/"default_values"> method takes care of flattening this datastructure to a simple hashref apprporate for the basic L<HTML::FormFu/"default_values"> method. You will learn how to use models later in Chapter V: L<HTML::FormFu::Manual::Models>.
