...
The C<update> method updates a foreign object specific to the model class with the values from the submitted form. With L<HTML::FormFu::Model::HashRef> it will create a hashref with the values from the form. With L<HTML::FormFu::Model::DBIC> it will take as an argument to update a C<DBIx::Class::Result> object, and its values in the database will be updated with the ones submitted from the form. Here is an example of how to use C<update> in conjunction with C<default_values>: 
...
Use the C<model> method of the L<HTML::FormFu> object to specify which model class you want to load:
...
Although a form is normally associated with a particular L<DBIx::Class> result class, you do not normally have to specify which class that is. Instead, you pass an object of that class as the argument to C<default_values> or C<update>. L<HTML::FormFu> will work happiliy as long as you have given it an object which has columns and relationships corresponding to the ones you have specified in the form definition.
...
L<HTML::FormFu::Model::DBIC> has two complementary options that affect how rows are selected: C<condition> and C<attributes>. They are passed as the first and second argument to L<DBIx::Class::ResultSet::search> respectively. That is, if you have the following C<model_config> specification for an element:
...
In the above examples we used lists of predifined options to set data related to other tables. Sometimes, however, we need to be able to edit directly data that is in a different table. The way this works in L<HTML::FormFu::Model::DBIC> is again very straightforward - to edit fields from a different table, you need to place those fields in a block which has a C<nested_name> property that corresponds to the respective relationship name. L<HTML::FormFu::Model::DBIC> will recognize that these fields belong to the related table and will update them accordingly.

Let us look at some examples now. Imagine that we have an C<author> table in our database with informatin about books, and we have a separate table C<address> that holds the address of each author. Our schema classes will look like this:
...
When you use C<nested_name> on a block, the value of C<nested_name> will be prepended to the name of each field within that block. This allows L<HTML::FormFu::Model::DBIC> to identify the fields that belong to foreign tables and update them accordingly. The above form will be rendered as follows:
...
The last and most complicated example that we will look into involves C<has_many> relationships, where we need to work with multiple rows from the related table at once. If we elaborate on the above scenario, imagine that we must modify our database to allow multiple addresses to be specified for each author. The new classes will have the following structure:
...
As above, the C<nested_name> option of the L<HTML::FormFu::Element::Repeatable> block must be set to the name of the respective relationship (C<addresses> in our case).

=item *

When you list the fields from the related table in the L<HTML::FormFu::Element::Repeatable> block you must include that table's primary key as a Hidden field. Since we are working with multiple rows this is necessary to identify rows by id when updating the form.

=item * 

The  L<HTML::FormFu::Element::Repeatable> block's C<increment_field_names> must be set to true (it is true by default). We will look into the effects of this option shortly.

=item *

And last but not least, you must include a Hidden field specifying the number of rows from the related table that the form will contain, i.e. how many times the Repeatable block will be repeated. This field must be placed outside the Repeatable block, and be referenced from within the Repeatable block via the C<counter_name> option. For more information, check the documentation on L<HTML::FormFu::Element::Repeatable>.
...
Notice how the use of the C<increment_field_names> option makes each field from the related table numbered:
...
If you want to add to the C<has_many> relationship (i.e. add a new address row for this author), you can use the C<empty_rows> option of the C<model_config> section of the L<HTML::FormFu::Element::Repeatable> block to display the form with the specified number of additional empty rows at the end of the block. Alternatively you can use javascript to add dynamically a new element, e.g. by copying the least element in the L<HTML::FormFu::Element::Repeatable> block and incrementing its counter. And in order to delete an existing element you can add a checkbox with a C<delete_if_true> option of its C<model_config> section set to C<1>. Those addresses with this checkbox checked will be deleted upon form submission. Alternatively you can use javascript to remove this field from the form and decrement the counters of the other elements respectively.
