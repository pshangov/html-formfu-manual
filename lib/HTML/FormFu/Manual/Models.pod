...
=head2 Working with many_to_many relationships

Some time after we have implemented our database, however, a new requirement comes up that more than one genre may be spefied for each book. To achieve that, we will need to add an intermediary table to keep track of the links between books and genres.
