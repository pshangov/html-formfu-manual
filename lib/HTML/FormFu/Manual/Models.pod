...
Let us look at some examples now. Imagin that we have an C<author> table in our database with informatin about books, and we have a separate table C<address> that holds the address of each author. Our schema classes will look like this:
...
The last and most comlicated example that we will look into involves C<has_many> relationships, where we need to work with multiple rows from the related table at once. If we elaborate on the above scenario, imagine that we must modify our database to allow multiple addresses to be specified for each author. The new classes will have the following structure:
...
The L<HTML::FormFu::Element::Repeatable> block must contain a Hidden field corresponding to the foreign key in the related table. Since we are working with multiple rows from the related table, this helps identify rows when updating the form.
...
Outside the L<HTML::FormFu::Element::Repeatable> block we must include a L<HTML::FormFu::Element::Hidden> field, which will contain the number of repetitions of the L<HTML::FormFu::Element::Repeatable> block. Then we must specify the name of this field in the C<counter_name> option of the L<HTML::FormFu::Element::Repeatable> block. This field is populated and used internally by L<HTML::FormFu::Model::DBIC>.
...
Notice how the use of the C<increment_field_names> option makes each row from the related table numbered:
