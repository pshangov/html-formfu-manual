...
You have to pass to C<config_callback> a hashref in a format compliant with L<Data::Visitor::Callback/CALLBACKS>. The keys are the type of data that you want to process, and the value is the coderef that will be executed. Inside the coderef, C<$_> variable will be set to the data that is currently being processed.
...
Some of the methods in L<HTML::FormFu> make no sense in a configuration file, since they require something to be done with their return value. These are for example methods that introspect the state of the form (C<sumtitted>, C<valid>, C<param>), methods that introspect the structure of the form (
C<get_element>, C<get_filter>, the constructor (C<new>), etc. 
